...
func DetectObjectLocal(modelId string, samplePath string, accessToken string) ([]byte, error) {
	base64 := getBase64String(samplePath)
	client := &http.Client{}
	body := &bytes.Buffer{}
	writer := multipart.NewWriter(body)
	...
	req, err := http.NewRequest("POST", DETECT_URL, body)
	req.Header.Set("Content-Type", writer.FormDataContentType())
...
func DetectObject(modelId string, sampleLocation string, accessToken string) ([]byte, error){
	client := &http.Client{}
	body := &bytes.Buffer{}
	writer := multipart.NewWriter(body)
